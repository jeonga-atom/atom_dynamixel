...
groupSyncWrite_GP = GroupSyncWrite(portHandler, packetHandler, ADDR_XM430_GOAL_POSITION, LEN_XM430_GOAL_POSITION)

# Initialize GroupSyncRead instace for Present Position
groupSyncRead_PP = GroupSyncRead(portHandler, packetHandler, ADDR_XM430_PRESENT_POSITION, LEN_XM430_PRESENT_POSITION)

# Initialize GroupSyncWrite instance for Goal Velocity
groupSyncWrite_GV = GroupSyncWrite(portHandler, packetHandler, ADDR_XM430_GOAL_VELOCITY, LEN_XM430_GOAL_VELOCITY)

# Initialize GroupSyncRead instace for Present Velocity
groupSyncRead_PV = GroupSyncRead(portHandler, packetHandler, ADDR_XM430_PRESENT_VELOCITY, LEN_XM430_PRESENT_VELOCITY)
...
list_of_DXL = [DXL1_ID, DXL2_ID, DXL5_ID, DXL6_ID]
...
if execute_control:
    ...
    change_OP_mode(ID_c= DXL1_ID, Mode_set_to= "Position")
    change_OP_mode(ID_c= DXL2_ID, Mode_set_to= "Velocity")
    change_OP_mode(ID_c= DXL5_ID, Mode_set_to= "Position")
    change_OP_mode(ID_c= DXL6_ID, Mode_set_to= "Velocity")
...
while execute_control:
    ...
    if controlling_key == chr(0x77): # w key(+Velocity↑)
        Target_Velocity += 4
        if Target_Position < 2048:
            Target_Position += 4
        elif Target_Position > 2048:
            Target_Position -= 4
        else:
            pass
    ...
    print(f"Target Position: {Target_Position:>4}\tTarget Velocity: {Target_Velocity:>3}")
    ...
    gBulkWrite(ID_list= list_of_DXL,
               ADDR_list= [ADDR_XM430_GOAL_POSITION, ADDR_XM430_GOAL_VELOCITY, ADDR_XM430_GOAL_POSITION, ADDR_XM430_GOAL_VELOCITY],
               addr_LEN_list= [LEN_XM430_GOAL_POSITION, LEN_XM430_GOAL_VELOCITY, LEN_XM430_GOAL_POSITION, LEN_XM430_GOAL_VELOCITY],
               PARAM_list= [bytes_Position, bytes_Velocity, bytes_Position, bytes_Velocity])
