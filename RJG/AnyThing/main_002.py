...
target_camera_pose  = _DATA["target_camera_pose"]
guide_camera_pose   = _DATA["guide_camera_pose"]
...
dir_seg_guide_py         = _DATA["dir_seg_guide_py"]
dir_seg_target_PCA_py    = _DATA["dir_seg_target_PCA_py"]
...
def T_load_detections(json_path):
    ...
    load_json_file = []
    while len(load_json_file) != 17 or (None in load_json_file):
        try:
...
def G_load_detections(json_path):
    ...
    load_json_file = [[], [], []]
    while len(load_json_file) != 3 or (not all(len(data) == 17 for data in load_json_file)):
        try:
            with open(json_path, "r", encoding="utf-8") as file:
                load_json_file = json.load(file)
        except FileNotFoundError:
...
def run_other_py(Python_file_dir):
    subprocess.run(['python3', Python_file_dir])
...
def target_camera(cam_to_obj="target"):
    print(f"\n\nTake a picture({cam_to_obj}).")
    global Init_check
    if cam_to_obj == "target":
        camera_pose = target_camera_pose
    elif cam_to_obj == "guide":
        camera_pose = guide_camera_pose
    else: # default
        camera_pose = target_camera_pose
    cmd_data = f"""
...
def target_ready(_target_data):
    ...
    print(_target_data)  # [x, y, angle_deg, class]
    target_cls = _target_data[16]
    Moving_pose = _target_data[:16]
...
def guide_move(_guide_data):
    ...
    guide_cls = _guide_data[int(target_cls/2)][16]  # [x, y, angle_deg, class]
    print(f"Target class: {target_cls}\nGuide class: {guide_cls}")
    Moving_pose = _guide_data[int(target_cls/2)][:16]
    Moving_pose[11] = guide_move_height
    cmd_data = f"""
    robot.SetVelocity({normal_vel})
    robot.movel({Moving_pose})
...
if __name__ == "__main__":
    try:
        ...
        # 3인칭 카메라로 촬영 신호를 주고, 잠시 대기(3인칭 카메라 실행 코드 넣어야 함)
        run_other_py(dir_seg_guide_py)
        guide_data = G_load_detections(json_path=guide_json_dir)
        # 가이드 최초 1회 촬영 후 계속 동일 위치값 사용
        while any(guide_data[index][16] != cls_Guides[index] for index in range(3)) or any(None in data for data in guide_data):
        ...
        while Processing_now:
            ...
            # 그리퍼 카메라로 촬영 신호를 주고, 잠시 대기(그리퍼 카메라 실행 코드 넣어야 함)
            run_other_py(dir_seg_target_PCA_py)
            tmp_target_data = T_load_detections(json_path=result_json_dir)
            # 촬영 마칠 때까지 대기 필요, 대기 과정 추가하기
            while tmp_target_data == target_data:
                print("Target object data(1 × 16 Matrix, class) is same as previous one.")
                run_other_py(dir_seg_target_PCA_py)
                tmp_target_data = T_load_detections(json_path=result_json_dir)
                debugging_delay(0.05)
