...
#                 elif gimbal_angle_y >0 :
#                     if gimbal_angle_y == 7:
#                         goal_pos = 2048 + (7/360)*4096*1
#                         packetHandler.write4ByteTxRx(portHandler,1, 116, int(goal_pos))

#                     elif gimbal_angle_y == 14:
#                         goal_pos = 2048 + (7/360)*4096*2
#                         packetHandler.write4ByteTxRx(portHandler,1, 116, int(goal_pos))

#                     elif gimbal_angle_y == 21:
#                         goal_pos = 2048 + (7/360)*4096*3
#                         packetHandler.write4ByteTxRx(portHandler,1, 116, int(goal_pos))

#                 elif gimbal_angle_y <10 :
#                     if gimbal_angle_y == -7:
#                         goal_pos = 2048 - (7/360)*4096*1
#                         packetHandler.write4ByteTxRx(portHandler,1, 116, int(goal_pos))
    ...
    while True:
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                if gimbal_angle_y == 0 :
                    packetHandler.write4ByteTxRx(portHandler, 1, 116, 2048)

                elif gimbal_angle_y > 0:
                    if gimbal_angle_y == 7:
                        new_goal_pos = goal_pos + (7/360)*4096 * 1

                    elif gimbal_angle_y == 14:
                        new_goal_pos = goal_pos + (7/360)*4096 * 2

                    elif gimbal_angle_y == 21:
                        new_goal_pos = goal_pos + (7/360)*4096 * 3
                ...
                # 새로운 목표 위치를 다이나믹셀에 씁니다.
                if new_goal_pos != goal_pos:  # 위치가 변경된 경우에만 쓰기
                    packetHandler.write4ByteTxRx(portHandler, 1, 116, int(new_goal_pos))
                    goal_pos = new_goal_pos  # 현재 목표 위치를 업데이트
