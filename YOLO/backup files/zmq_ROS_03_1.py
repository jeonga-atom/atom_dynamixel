...
DEFAULT_ROS_DRIVE = {
    ...
    'fire_1': '/drive/fire_start_1',
    'fire_2': '/drive/fire_start_2',
    'fire_3': '/drive/fire_start_3',
    'fire_4': '/drive/fire_start_4',
...
class DetectionNode(Node):
    def __init__(self):
        ...
        self.fire_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['fire_1'],
            self.fire_1_start_callback,
            10)
        
        self.fire_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['fire_2'],
            self.fire_2_start_callback,
            10)
        
        self.fire_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['fire_3'],
            self.fire_3_start_callback,
            10)
        
        self.fire_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['fire_4'],
            self.fire_4_start_callback,
            10)
    ...
    def fire_1_start_callback(self, msg):
        try:
            self.socket_drive.send_string("fire_start_1")
            self.get_logger().info('ZMQ 발행: fire_start_1')
        except Exception as e:
            self.get_logger().error(f'ZMQ fire_start 발행 실패: {str(e)}')

    def fire_2_start_callback(self, msg):
        try:
            self.socket_drive.send_string("fire_start_2")
            self.get_logger().info('ZMQ 발행: fire_start_2')
        except Exception as e:
            self.get_logger().error(f'ZMQ fire_start 발행 실패: {str(e)}')

    def fire_3_start_callback(self, msg):
        try:
            self.socket_drive.send_string("fire_start_3")
            self.get_logger().info('ZMQ 발행: fire_start_3')
        except Exception as e:
            self.get_logger().error(f'ZMQ fire_start 발행 실패: {str(e)}')

    def fire_4_start_callback(self, msg):
        try:
            self.socket_drive.send_string("fire_start_4")
            self.get_logger().info('ZMQ 발행: fire_start_4')
        except Exception as e:
            self.get_logger().error(f'ZMQ fire_start 발행 실패: {str(e)}')
