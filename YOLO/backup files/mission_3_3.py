...
try:
    while True:
        ...
        if detection_enabled['fire'] and current_time - fire_last_capture_time >= fire_capture_interval:
            ...
            detected_on = False       # fire on
            detected_off = False      # fire off
            ...
            if fire_status in ["on", "off"]:
                ...
                print("No valid detection in this capture (not counted)")

            fire_last_capture_time = current_time

            if fire_capture_count >= required_detections:     # 캡처 횟수 지정
