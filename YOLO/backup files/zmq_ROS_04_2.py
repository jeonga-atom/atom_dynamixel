...
class DetectionNode(Node):
    def __init__(self):
        ...
        self.safebox_publisher = self.create_publisher(
            Float32MultiArray, 
        ...
        # ROS 서브스크라이버 생성
        self.safebox_publisher_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['safebox'],
            self.safebox_start_again_callback,
            10)
        
        self.human_1_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['human_1'],
            self.human_start_1_callback,
            10)
        
        self.human_2_subscriber = self.create_subscription(
            Empty,
            DEFAULT_ROS_DRIVE['human_2'],
            self.human_start_2_callback,
            10)
        
        self.human_3_subscriber = self.create_subscription(
            Empty,
            ...
            try:
                ...
                while self.socket_detection in socks and socks[self.socket_detection] == zmq.POLLIN:
                    ...
                    if topic in DEFAULT_ROS_DETECTED.values():  # final 토픽만 처리 (temp 무시)
                        if topic == "/object_detection/safebox":
                            safebox_data = pickle.loads(data)
                            msg = Float32MultiArray()
                            msg.data = [float(x) for x in safebox_data]  # 리스트를 float 리스트로 변환
                            self.safebox_publisher.publish(msg)
                            self.get_logger().info(f"ROS 발행: safebox_detected (x: {float(msg.data[0]):.2f}, depth: {float(msg.data[1]):.2f}m)")
                        ...
                        elif topic == "/object_detection/human/not_detected":
                            msg = Empty()
                            self.human_blue_publisher.publish(msg)
                            self.get_logger().info('ROS 발행: not_detected')
