...
camera_matrix = np.array([[623.54, 0, 331.54],
                          [0, 626.04, 246.28],
                          [0, 0, 1]])
dist_coeffs = np.array([0.1156, -0.0068, -0.0022, 0.0046, -0.5229])
    ...
    while True:
        ...
        for result in results:
            ...
            for (x1, y1, x2, y2), conf, class_id in zip(boxes, confidences, class_ids):
                ...
                bev_x = int(BEV_WIDTH // 2 + obj_x * SCALE)
                bev_y = int(BEV_HEIGHT - obj_y * SCALE)

                # 범위 제한 (이미지 크기를 넘지 않도록)
                bev_x = np.clip(bev_x, 0, BEV_WIDTH)
