# 이 코드는 원하는 거리를 재기 위해 마우스 드래그해서 거리를 측정할 수 있는 코드.
...
class ARC:
    def __init__(self):
        ...
        #config = rs.config()
        #config.enable_device_from_file(bag, False) -> 이 줄은 .bag 파일을 재생하는 부분이므로 실시간 카메라를 실행하지 않음. 따라서 주석처리함
        #config.enable_all_streams()

        # 실시간 카메라 스트림만 사용하도록 config 설정
        config = rs.config()
        # 예: 컬러 640×480 @30fps, 깊이 640×480 @30fps
