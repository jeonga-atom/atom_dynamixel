...
class DDSM115_CMD():
    ...
    # 생성자 메소드 선언
    def __init__(self, motor_USBport_name: str, motor_setting_ID: int, motor_Coordinates: list, motor_Current_Limit: int = 7, motor_Velocity_Limit: int = 100, Output_for_User: bool = False):
        ...
        self.cSerial = serial.Serial(
            port = str(motor_USBport_name), # USB 연결 시 인식되는 /dev/ttyACM 에 물리적으로 연결한 위치(순서)에 맞게 번호(motor_USBport_num) 부여할 것. 모터의 ID와는 별개의 변수이므로 유의할 것!
            baudrate = 115200,
...
testDDSM = DDSM115_CMD(motor_USBport_name="/dev/ttyACM0", motor_setting_ID=1, motor_Coordinates=[-1, 1], Output_for_User=True)

# 모터를 처음에 켰을 때 한 번 실행해서 초기화하기(한 번만 주석 해제해서 실행)
# testDDSM.init_motor_Buffer()
# testDDSM.set_ID(motor_ID=testDDSM.cMotor_ID)
# testDDSM.check_ID()
# 여기까지 초기화
...
# testDDSM.init_motor_Buffer()
# time.sleep(30)
# testDDSM.Operate_Motor_Velocity_Mode(motor_ID=1, raw_motor_Velocity=5)
# time.sleep(10)
# testDDSM.Operate_Motor_Velocity_Mode(motor_ID=1, raw_motor_Velocity=0)
# 구동부 끝
