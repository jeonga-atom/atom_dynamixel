...
import Dynmxl_init_and_func as dxlATOM
...
##### ##### ##### ##### ##### 메인 함수 ##### ##### ##### ##### #####
dxlATOM.Dynmxl_Start_process_for_ATOM()
...
try:
    ...
    while not stop_flag:
        ...
        if type(depth) == float and (not(math.isnan(depth)) and depth > 0.0):
            if LIMIT_DISTANCE <= depth <= 1.1:
                temp_Velo = 200 * math.sqrt((depth - LIMIT_DISTANCE) / 0.8)
            elif 0.0 < depth < LIMIT_DISTANCE:
                temp_Velo = -200 * math.sqrt((LIMIT_DISTANCE - depth) / 0.5) # 후진 적용 시
                # temp_Velo = 0
            else:
                temp_Velo = 0
            # print(type(depth))
        ...
        dxlATOM.Dynmxl_process_for_ATOM(Outer_Wheel_Angle_in4096=temp_Angle, Outer_Wheel_Velocity_in200=temp_Velo, Gimbal_Roll_in4096=new_position, Gimbal_Pitch_in4096=temp_pitch)
    ...
    dxlATOM.Dynmxl_Finish_process_for_ATOM()
