...
import DXL_DDSM_Const as atomConst
import DDSM_ctrl_class_wDXL as ddsm
...
portHandler = PortHandler(atomConst.ATOM_DXL_DEVICENAME)

# Initialize PacketHandler instance
# Set the protocol version
# Get methods and members of Protocol1PacketHandler or Protocol2PacketHandler
packetHandler = PacketHandler(atomConst.ATOM_DXL_PROTOCOL_VER)

# Initialize GroupSyncWrite instance for Goal Position
groupSyncWrite_POS = GroupSyncWrite(portHandler, packetHandler, atomConst.ADDR_GOAL_POSITION, atomConst.LEN_GOAL_POSITION)

# Initialize GroupSyncRead instace for Present Position
groupSyncRead_POS = GroupSyncRead(portHandler, packetHandler, atomConst.ADDR_PRESENT_POSITION, atomConst.LEN_PRESENT_POSITION)

# Initialize GroupSyncWrite instance for Goal Velocity
groupSyncWrite_VEL = GroupSyncWrite(portHandler, packetHandler, atomConst.ADDR_GOAL_VELOCITY, atomConst.LEN_GOAL_VELOCITY)

# Initialize GroupSyncRead instace for Present Velocity
groupSyncRead_VEL = GroupSyncRead(portHandler, packetHandler, atomConst.ADDR_PRESENT_VELOCITY, atomConst.LEN_PRESENT_VELOCITY)


# Initialize DDSM motor instances
DDSM_FrontLeft = ddsm.DDSM115_CMD(motor_USBport_num=atomConst.ATOM_DDSM_USB_PORT_LIST[0], motor_setting_ID=atomConst.DDSM_ID_VEL[0], motor_Coordinates=[-1, 1], motor_Velocity_Limit=atomConst.DDSM_MAX_VEL_LIMIT, Output_for_User=True)
DDSM_FrontRight = ddsm.DDSM115_CMD(motor_USBport_num=atomConst.ATOM_DDSM_USB_PORT_LIST[1], motor_setting_ID=atomConst.DDSM_ID_VEL[1], motor_Coordinates=[1, 1], motor_Velocity_Limit=atomConst.DDSM_MAX_VEL_LIMIT, Output_for_User=True)
DDSM_RearLeft = ddsm.DDSM115_CMD(motor_USBport_num=atomConst.ATOM_DDSM_USB_PORT_LIST[2], motor_setting_ID=atomConst.DDSM_ID_VEL[2], motor_Coordinates=[-1, 0], motor_Velocity_Limit=atomConst.DDSM_MAX_VEL_LIMIT, Output_for_User=True)
DDSM_RearRight = ddsm.DDSM115_CMD(motor_USBport_num=atomConst.ATOM_DDSM_USB_PORT_LIST[3], motor_setting_ID=atomConst.DDSM_ID_VEL[3], motor_Coordinates=[1, 0], motor_Velocity_Limit=atomConst.DDSM_MAX_VEL_LIMIT, Output_for_User=True)
...
def Dynmxl_Torque(Motor_ID: int, ENabled_or_DISabled: int):
    ...
    dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, Motor_ID, atomConst.ADDR_TORQUE_VALUE, ENabled_or_DISabled)
...
def Dynmxl_Read_PresentValue(ID_: int):
    ...
    while True:
        ...
        if Present_OP_Mode == 1:
            ...
            if not abs(atomConst.FORWARD_POS - dxl_present_Value) > 0:
                print()
                break
...
def Dynmxl_GrpSyncRead_addParam(Motor_ID: int, motor_Operating_Type):
    ...
    if motor_Operating_Type == atomConst.OP_VALUE_POSITION:
        dxl_addparam_result = groupSyncRead_POS.addParam(Motor_ID)
    elif motor_Operating_Type == atomConst.OP_VALUE_VELOCITY:
...
def Dynmxl_GrpSyncRead_txRxPacket(motor_ID: int, motor_Operating_Type: int):
    ...
    if motor_Operating_Type == atomConst.OP_VALUE_POSITION:
        return groupSyncRead_POS.getData(motor_ID, atomConst.ADDR_PRESENT_POSITION, atomConst.LEN_PRESENT_POSITION)
    elif motor_Operating_Type == atomConst.OP_VALUE_VELOCITY:
        return groupSyncRead_VEL.getData(motor_ID, atomConst.ADDR_PRESENT_VELOCITY, atomConst.LEN_PRESENT_VELOCITY)
    else:
        quit(f"[Error in motor_Operating_Type of Dynmxl_GrpSyncRead_txRxPacket()]\nDynamixel (ID: {motor_ID:02d}) will be received unexpected operating mode. Received value: {motor_Operating_Type}\nThis code use only two numbers(modes). > 1: Velocity / 3: Position")
...
def Dynmxl_Initialization(Setup_List: list):
    for ID_index in Setup_List:
        for Data_index in ID_index[1]:
            ...
            if Data_index[0] == atomConst.ADDR_GOAL_POSITION:
                while True:
                    dxl_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, ID_index[0], atomConst.ADDR_PRESENT_POSITION)
                    ...
                    if not abs(Data_index[2] - dxl_present_position) > atomConst.DXL_POS_THRESHOLD:
                        print()
                        break

            elif Data_index[0] == atomConst.ADDR_GOAL_VELOCITY:
                while True:
                    dxl_present_velocity, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, ID_index[0], atomConst.ADDR_PRESENT_VELOCITY)
...
def Radius_calc(OuterWheel_Angle_in4096: int):
    ...
    if 180.0 - atomConst.MAXIMUM_STEERING_ANG <= Outer_Angle_to360 < 180.0: # 정면 좌측부터 정면 직전
        Virtual_Radius_from_OuterLine = (atomConst.WHEELBASE / math.tan(math.radians(180 - Outer_Angle_to360)))
        Virtual_Radius_from_CenterLine = Virtual_Radius_from_OuterLine - atomConst.HALF_TREAD
        Steering_Direction = atomConst.DIR_LEFT

    elif 180.0 < Outer_Angle_to360 <= 180.0 + atomConst.MAXIMUM_STEERING_ANG: # 정면 우측부터 정면 직전
        Virtual_Radius_from_OuterLine = (atomConst.WHEELBASE / math.tan(math.radians(Outer_Angle_to360)))
        Virtual_Radius_from_CenterLine = Virtual_Radius_from_OuterLine - atomConst.HALF_TREAD
        Steering_Direction = atomConst.DIR_RIGHT

    elif (Outer_Angle_to360 < 180.0 - atomConst.MAXIMUM_STEERING_ANG) and not(Outer_Angle_to360 in [0.0, 90.0, 180.0, 270.0]): # 90도 배수를 제외한 나머지 좌측 방향
        Virtual_Radius_from_OuterLine = (atomConst.WHEELBASE / math.tan(math.radians(atomConst.MAXIMUM_STEERING_ANG)))
        Virtual_Radius_from_CenterLine = Virtual_Radius_from_OuterLine - atomConst.HALF_TREAD
        Steering_Direction = atomConst.DIR_LEFT

    elif (180.0 + atomConst.MAXIMUM_STEERING_ANG < Outer_Angle_to360) and not(Outer_Angle_to360 in [0.0, 90.0, 180.0, 270.0]): # 90도 배수를 제외한 나머지 우측 방향
        Virtual_Radius_from_OuterLine = (atomConst.WHEELBASE / math.tan(math.radians(atomConst.MAXIMUM_STEERING_ANG)))
        Virtual_Radius_from_CenterLine = Virtual_Radius_from_OuterLine - atomConst.HALF_TREAD
        Steering_Direction = atomConst.DIR_RIGHT
...
def Angle_calc(Wheel_Coordinates: list, Steering_Direction: int, Radius_from_TurningPoint_to_CenterLine: float, Vertical_Length_to_Wheel_from_TurningPoint: float, Horizontal_Width_to_Wheel_from_CenterLine: float = atomConst.HALF_TREAD):
    '''
    ...
    if Steering_Direction == atomConst.DIR_FORWARD:
        Steering_Angle_to4096 = 2048
    
    elif Steering_Direction == atomConst.DIR_LEFT or atomConst.DIR_RIGHT:
        if Wheel_Y == 0:
            Steering_Angle_to4096 = 2048
        else: # Wheel_Y == ±1:
            ...
            Steering_Angle_to4096 = round(atomConst.FORWARD_POS + ((Steering_Direction * Steering_Alpha) * (4096 / 360))) # Direction numbers(Left: -1 and Right: +1) were defined in reverse, so we should minus from FORWARD_POS. But when we use Dynamixel in real, we can think and calculate direction easily.
...
def Velo_calc(Target_Velocity_of_theMost_OuterWheel_in200: int, Wheel_Coordinates: list, Steering_Direction: int, Radius_from_TurningPoint_to_CenterLine: float, Vertical_Length_to_Wheel_from_TurningPoint: float, Horizontal_Width_to_Wheel_from_CenterLine: float = atomConst.HALF_TREAD):
    '''
    ...
    if Steering_Direction == atomConst.DIR_FORWARD:
        ...
        Adjusted_Wheel_Velocity = Target_Velocity_of_theMost_OuterWheel_in200 * (Length_from_Wheel_to_TurningPoint / (math.sqrt((Radius_from_TurningPoint_to_CenterLine + max(atomConst.LIST_HALF_TREAD))**2 + max(atomConst.LIST_WHEELBASE)**2)))
...
def Motor_TxRx_for_ATOM(Steering_Direction_data: int, Turning_Radius_data: float, Velocity_data: int, Gimbal_Roll_data: int = atomConst.INIT_DXL_POS, Gimbal_Pitch_data: int = atomConst.INIT_DXL_POS):
    groupSyncWrite_POS.clearParam()
    groupSyncWrite_VEL.clearParam()

    FrontLeft_Angle_in4096 = Angle_calc(Wheel_Coordinates=[-1, 1], Steering_Direction=Steering_Direction_data, Radius_from_TurningPoint_to_CenterLine=Turning_Radius_data, Vertical_Length_to_Wheel_from_TurningPoint=atomConst.WHEELBASE, Horizontal_Width_to_Wheel_from_CenterLine=atomConst.HALF_TREAD)
    FrontRight_Angle_in4096 = Angle_calc(Wheel_Coordinates=[1, 1], Steering_Direction=Steering_Direction_data, Radius_from_TurningPoint_to_CenterLine=Turning_Radius_data, Vertical_Length_to_Wheel_from_TurningPoint=atomConst.WHEELBASE, Horizontal_Width_to_Wheel_from_CenterLine=atomConst.HALF_TREAD)
    
    FrontLeft_Velo_inDDSM = Velo_calc(Target_Velocity_of_theMost_OuterWheel_in200=Velocity_data, Wheel_Coordinates=[-1, 1], Steering_Direction=Steering_Direction_data, Radius_from_TurningPoint_to_CenterLine=Turning_Radius_data, Vertical_Length_to_Wheel_from_TurningPoint=atomConst.WHEELBASE, Horizontal_Width_to_Wheel_from_CenterLine=atomConst.HALF_TREAD)
    ...
    for POS_index in range(len(atomConst.DXL_ID_POS)):
        groupSyncWrite_POS.addParam(atomConst.DXL_ID_POS[POS_index], Allocate_DXL_data(List_of_Angle_in4096[POS_index]))

    '''
    for VEL_index in range(len(atomConst.DDSM_ID_VEL)):
        pass
        # groupSyncWrite_VEL.addParam(atomConst.DDSM_ID_VEL[VEL_index], Allocate_DXL_data((List_of_Velo_inDDSM[VEL_index] & 0xFFFFFFFF))) # 0xFFFFFFFF makes negative value to deliver correctly.
    '''
...
def Motor_Start_process_for_ATOM():
    for turnon_DXL_ID_index in atomConst.DXL_ID_POS:
        Dynmxl_Torque(Motor_ID=turnon_DXL_ID_index, ENabled_or_DISabled=1)

    DDSM_turn_on_results = 0
    DDSM_check_results = []
    for turnon_DDSM_ID_index in atomConst.DDSM_ID_VEL:
    ...
    if(DDSM_turn_on_results == 0):
        Motor_TxRx_for_ATOM(Steering_Direction_data=atomConst.DIR_FORWARD, Turning_Radius_data=0, Velocity_data=0)
    else:
...
def Motor_Finish_process_for_ATOM():
    ...
    for repeat_ID_index in atomConst.DDSM_ID_VEL:
        DDSM_group[repeat_ID_index].Brake_Motor(motor_ID=DDSM_group[repeat_ID_index].cMotor_ID)
        # DDSM 속도 완전 0으로 만들기

    for turnoff_DDSM_ID_index in atomConst.DDSM_ID_VEL:
        DDSM_group[turnoff_DDSM_ID_index].init_motor_Buffer()
        # 종료 전 DDSM 버퍼 초기화...
        
    for turnoff_DXL_ID_index in atomConst.DXL_ID_POS:
...
# Set port baudrate
if portHandler.setBaudRate(atomConst.ATOM_DXL_BAUDRATE):
...
Dynmxl_Initialization(Setup_List=atomConst.DXL_INIT_SETUP)
DDSM_Initialization(Setup_ID_list=atomConst.DDSM_ID_VEL)
...
if __name__ =="__main__":
    R = Radius_calc(1537)
    A = Angle_calc(Wheel_Coordinates=[1, 1], Steering_Direction=atomConst.DIR_LEFT, Radius_from_TurningPoint_to_CenterLine=0.2, Vertical_Length_to_Wheel_from_TurningPoint=atomConst.WHEELBASE, Horizontal_Width_to_Wheel_from_CenterLine=atomConst.HALF_TREAD)
    V = Velo_calc(Target_Velocity_of_theMost_OuterWheel_in200=200,Wheel_Coordinates=[1, 1], Steering_Direction=atomConst.DIR_RIGHT, Radius_from_TurningPoint_to_CenterLine=0.2, Vertical_Length_to_Wheel_from_TurningPoint=atomConst.WHEELBASE, Horizontal_Width_to_Wheel_from_CenterLine=atomConst.HALF_TREAD)
