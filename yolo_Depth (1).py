...
try:
    while True:
        ...
        depth_image = np.asanyarray(aligned_depth_frame.get_data()) #ndarray(n차원행렬)로 변환, 이번에 뎁스에 대한 프레임을 n차원행렬화, 이렇게 수치화를 시켜야 속도가 향상된다.
        ...
        for result in results: #영상처리에서 이 문장은 필수적
            ...
            for box in boxes:
                ...
                depth_object_name = f"{model.names[int(class_id)]}, {object_depth:.4f}m"
